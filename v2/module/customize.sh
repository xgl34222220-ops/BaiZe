...
ui_print "- 安装白泽 v2 Alpha 42.9 智能扫描统一主题版"
ui_print "- 智能扫描页已迁移 Compose，跟随 MIUIx、Monet、明暗模式与 AMOLED"
ui_print "- 清理过程中隐藏过期的一键清理、重新扫描与详情按钮"
ui_print "- 应用缓存、安装包及智能扫描清理继续只消费刚才生成的快照"
ui_print "- 清理页会读取 Root 引擎真实阶段、目标路径与停止状态"
...
chmod 0755 "$MODPATH/cleaner.sh" "$MODPATH/native-cleaner.sh" "$MODPATH/cache-snapshot-clean.sh" "$MODPATH/apk-scanner.sh" "$MODPATH/apk-cleaner.sh" "$MODPATH/profile-cleaner.sh" 2>/dev/null
