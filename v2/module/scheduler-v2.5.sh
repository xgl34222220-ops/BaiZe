...
collect_scheduled_candidates() {
  ...
  now=$(date +%s); daily=0
  [ "$(bool_value daily_schedule_enabled)" = 1 ] && daily_cycle_info && daily=1
  for group in cache empty rules fragment deep organize; do
    ...
    elif [ "$group" != organize ] && [ "$(bool_value daily_schedule_enabled)" = 1 ]; then
      continue
    else
...
apply_skip_requests() {
  for file in "$SKIP_DIR"/*.request; do
    ...
    if group_spec "$group"; then
      ...
      if [ "$group" != organize ] && [ "$(bool_value daily_schedule_enabled)" = 1 ] && daily_cycle_info; then
