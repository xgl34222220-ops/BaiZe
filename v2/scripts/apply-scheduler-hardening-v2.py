...
replace_count(app, "    val cacheMinutes: Int = 60,", "    val cacheMinutes: Int = 1_440,")
replace_count(app, "    val emptyMinutes: Int = 60,", "    val emptyMinutes: Int = 1_440,")
replace_count(app, "    val rulesMinutes: Int = 360,", "    val rulesMinutes: Int = 1_440,")
replace_count(app, "    val fragmentMinutes: Int = 720,", "    val fragmentMinutes: Int = 4_320,")
replace_count(app, "    val screenOffOnly: Boolean = false,", "    val screenOffOnly: Boolean = true,")
replace_count(app, 'json.optInt("schedule_cache_hours", 1) * 60', 'json.optInt("schedule_cache_hours", 24) * 60')
replace_count(app, 'json.optInt("schedule_empty_hours", 1) * 60', 'json.optInt("schedule_empty_hours", 24) * 60')
replace_count(app, 'json.optInt("schedule_rules_hours", 6) * 60', 'json.optInt("schedule_rules_hours", 24) * 60')
replace_count(app, 'json.optInt("schedule_fragment_hours", 12) * 60', 'json.optInt("schedule_fragment_hours", 72) * 60')
replace_count(app, 'screenOffOnly = json.optInt("screen_off_only", 0) == 1,', 'screenOffOnly = json.optInt("screen_off_only", 1) == 1,')
...
replace_count(repository, '                "rules" -> 360', '                "cache", "empty", "rules" -> 1_440')
replace_count(repository, '                "fragment" -> 720', '                "fragment" -> 4_320')
replace_count(repository, '                else -> 60', '                else -> 1_440')
...
replace_count(scheduler, "SPEC_FALLBACK=1; SPEC_MODE=cache-auto", "SPEC_FALLBACK=24; SPEC_MODE=cache-auto")
replace_count(scheduler, "SPEC_FALLBACK=1; SPEC_MODE=empty-clean", "SPEC_FALLBACK=24; SPEC_MODE=empty-clean")
replace_count(scheduler, "SPEC_FALLBACK=6; SPEC_MODE=rules-clean", "SPEC_FALLBACK=24; SPEC_MODE=rules-clean")
replace_count(scheduler, "SPEC_FALLBACK=12; SPEC_MODE=fragment-clean", "SPEC_FALLBACK=72; SPEC_MODE=fragment-clean")
replace_count(
    scheduler,
...
thermal_test = r'''#!/usr/bin/env bash
set -euo pipefail
...
for helper in (
    ...
    "v2/scripts/patch-scheduler-hardening-fixer.py",
    "v2/scripts/apply-scheduler-hardening-v2.py",
):
