...
unzip -p "$OUTPUT" module.prop | grep -q '^version=v2.5.1$'
unzip -p "$OUTPUT" module.prop | grep -q '^versionCode=25001$'
unzip -p "$OUTPUT" customize.sh | grep -Fqx 'ui_print "- 正在安装白泽 v2.5.1"'
if unzip -p "$OUTPUT" customize.sh | grep -Eq 'v2\.5\.0|versionCode=25000|v2\.4\.0|versionCode=24000'; then
