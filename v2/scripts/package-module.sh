...
OUTPUT="$OUT/BaiZe-v2.5.5-Module.zip"
...
unzip -p "$OUTPUT" task-worker.sh | grep -q 'detached-root-worker-v2.5.5'
...
unzip -p "$OUTPUT" module.prop | grep -q '^version=v2.5.5$'
unzip -p "$OUTPUT" module.prop | grep -q '^versionCode=25005$'
unzip -p "$OUTPUT" customize.sh | grep -Fqx 'ui_print "- 正在安装白泽 v2.5.5"'
...
echo "已生成白泽 v2.5.5 深度不可变快照模块：$OUTPUT"
