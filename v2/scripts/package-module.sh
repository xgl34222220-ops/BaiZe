...
OUTPUT="$OUT/BaiZe-v2.6.0-Module.zip"
...
python3 "$ROOT/scripts/compile-deep-rules.py" "$REPO/config/deep.rules" "$STAGE/config/deep.rules" "$STAGE/config/rules.meta.env" --index "$STAGE/config/rules.index.jsonl"
...
unzip -l "$OUTPUT" | grep -q 'record-clean-event.sh'
...
unzip -p "$OUTPUT" task-worker.sh | grep -q 'detached-root-worker-v2.6.0'
...
unzip -l "$OUTPUT" | grep -q 'config/rules.index.jsonl'
unzip -p "$OUTPUT" config/rules.index.jsonl | grep -q '"ruleId":"legacy-'
...
if unzip -p "$OUTPUT" customize.sh | grep -Eq 'v2\.5\.[0-9]+|versionCode=250[0-9]+|v2\.4\.0|versionCode=24000'; then
...
expected_rules_sha=$(unzip -p "$OUTPUT" config/rules.meta.env | sed -n 's/^rules_sha256=//p' | tail -n 1)
actual_rules_sha=$(unzip -p "$OUTPUT" config/deep.rules | sha256sum | awk '{print $1}')
[ -n "$expected_rules_sha" ] && [ "$actual_rules_sha" = "$expected_rules_sha" ] || {
  echo "编译规则索引与运行时规则 SHA 不一致" >&2
  exit 1
}
