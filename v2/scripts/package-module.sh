...
OUTPUT="$OUT/BaiZe-v2.1.0-Module.zip"
...
unzip -p "$OUTPUT" module.prop | grep -q '^version=v2.1.0$'
unzip -p "$OUTPUT" module.prop | grep -q '^versionCode=22500$'
...
echo "已生成白泽 v2.1.0 正式版模块：$OUTPUT"