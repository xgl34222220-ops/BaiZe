...
OUTPUT="$OUT/BaiZe-v2.5.6-Module.zip"
...
unzip -p "$OUTPUT" task-worker.sh | grep -q 'detached-root-worker-v2.5.6'
...
unzip -p "$OUTPUT" module.prop | grep -q '^version=v2.5.6$'
unzip -p "$OUTPUT" module.prop | grep -q '^versionCode=25006$'
unzip -p "$OUTPUT" customize.sh | grep -Fqx 'ui_print "- 正在安装白泽 v2.5.6"'
if unzip -p "$OUTPUT" customize.sh | grep -Eq 'v2\.5\.5|versionCode=25005|v2\.5\.2|versionCode=25002|v2\.5\.1|versionCode=25001|v2\.5\.0|versionCode=25000|v2\.4\.0|versionCode=24000'; then
...
echo "已生成白泽 v2.5.6 深度不可变快照模块：$OUTPUT"
