...
# Stage 1: immutable runtime and release content.
...
package_text = package.read_text().replace("v2.5.5", "v2.5.6").replace("25005", "25006")
...
# Do not include one-time preparation files in the immutable release target.
Path(".github/workflows/prepare-v2.5.6.yml").unlink(missing_ok=True)
...
subprocess.run(
    [
        ...
        ".github/workflows/prepare-v2.5.6.yml",
        "v2/scripts/prepare-release-v2.5.6.py",
    ],
    check=True,
)
...
# Stage 2: publication workflow and contracts. The workflow tags the stage-1 target.
source = Path(".github/workflows/v2.5.5-release.yml").read_text()
workflow = (
    source.replace("v2.5.5", "v2.5.6")
    .replace("2.5.5", "2.5.6")
    .replace("25005", "25006")
    .replace("v255", "v256")
)
workflow = re.sub(r"BAIZE_RELEASE_TARGET_SHA: [0-9a-f]{40}", f"BAIZE_RELEASE_TARGET_SHA: {target}", workflow)
...
Path(".github/workflows/v2.5.6-release.yml").write_text(workflow)
...
subprocess.run(
    [
        ...
        "v2/tests/test-apk-retention-contract.sh",
        "v2/tests/test-release-v2.5.6-contract.sh",
    ],
    check=True,
)
subprocess.run(["git", "commit", "-m", "ci: add immutable BaiZe v2.5.6 release pipeline"], check=True)
subprocess.run(["git", "push", "origin", "HEAD:release/v2.5.6"], check=True)
print(f"prepared v2.5.6 release target {target}")
