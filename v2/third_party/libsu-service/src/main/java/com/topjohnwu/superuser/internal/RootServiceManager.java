...
public class RootServiceManager implements Handler.Callback {
    ...
    private class PendingBind {
        ...
        PendingBind(Intent intent, Executor executor, ServiceConnection connection, boolean daemon) {
            this.intent = intent;
    ...
    private void failStartup(boolean daemon, Object ticket) {
        ...
        flags &= ~(daemon ? DAEMON_EN_ROUTE : REMOTE_EN_ROUTE);
        List<PendingBind> failed = new ArrayList<>();
        Iterator<PendingBind> it = pendingTasks.iterator();
        while (it.hasNext()) {
    ...
    public Shell.Task createBindTask(Intent intent, Executor executor, ServiceConnection conn) {
        ...
        if (key.isDaemon()) daemonStartup = ticket; else remoteStartup = ticket;
        final Shell.Task launch;
        try {
    ...
    public void unbind(@NonNull ServiceConnection conn) {
        ...
        Iterator<PendingBind> pending = pendingTasks.iterator();
        while (pending.hasNext()) {
            if (pending.next().connection == conn) pending.remove();
        }
        boolean waitingDaemon = false, waitingRemote = false;
        for (PendingBind request : pendingTasks) {
            if (request.daemon) waitingDaemon = true; else waitingRemote = true;
        }
        if (!waitingDaemon) { flags &= ~DAEMON_EN_ROUTE; daemonStartup = null; }
        if (!waitingRemote) { flags &= ~REMOTE_EN_ROUTE; remoteStartup = null; }
